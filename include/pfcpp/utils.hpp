...
template <typename T> class GaussianNoise {
  public:
    GaussianNoise(T sigma = 1.0) : gen(std::random_device {}()), distr(0, sigma) {}

    T operator()() { return static_cast<T>(distr(gen)); }
...
template <typename T> class Uniform {
  public:
    Uniform(T a = 0.0, T b = 1.0) : gen(std::random_device {}()), distr(a, b) {}
...
template <typename T> class MultimodalDistribution {
  public:
...
inline auto normal_angle(double angle) {
    while (angle < 0)
        angle += 2 * Pi;
    while (angle > 2 * Pi)
        angle -= 2 * Pi;
