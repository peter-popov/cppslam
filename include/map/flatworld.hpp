...
struct Pose {
    ...
    operator ShapesMap::Point() const {
        return {x, y};
    }
    operator ShapesMap::Position() const {
        return ShapesMap::Position{x, y};
    }
};
...
auto measurement_with_coords(Pose p, ShapesMap &map, int num_rays, double max_range) {
    ...
    if (!map.is_occupied(coord)) {
        ...
        for (int i = 0; i < num_rays; ++i) {
            std::tie(distances[i], end_points[i]) =
                map.min_distance_towards(coord, p.heading + i * da, max_range, noise());
        }
    }
...
auto measurement(Pose p, ShapesMap &map, int num_rays, double max_range) {
    ...
    if (!map.is_occupied(coord)) {
        auto da = 2 * Pi / num_rays;
        for (int i = 0; i < num_rays; ++i) {
            std::tie(distances[i], std::ignore) =
                map.min_distance_towards(coord, p.heading + i * da, max_range);
        }
    }
    return distances;
}
