...
template <typename T> struct MockGenerator {
    MockGenerator(std::initializer_list<T> v) : numbers(v) {}

    T operator()() { return numbers[i++ % numbers.size()]; }
...
auto get_weight(const CustomTestParticle &p) {
    return p.other_field;
};
