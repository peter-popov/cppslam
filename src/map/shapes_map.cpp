...
template <typename Point> auto make_box(Point a, Point b) {
    return ShapesMap::Box(
        {std::min(bg::get<0>(a), bg::get<0>(b)), std::min(bg::get<1>(a), bg::get<1>(b))},
        {std::max(bg::get<0>(a), bg::get<0>(b)), std::max(bg::get<1>(a), bg::get<1>(b))});
}

template <typename P, typename T>
auto fast_intersect(T length, const P &p1, const P &p2, const P &p3, const P &p4, P &res, T &dist) {
    T x, y;
    ...
    dist = std::abs(d * length / f);

    return true;
}

auto tuplify(ShapesMap::Point p) { return std::make_tuple(bg::get<0>(p), bg::get<1>(p)); }

template <typename T> ShapesMap::Point untuplify(T p) {
    return {std::get<0>(p), std::get<1>(p)};
}

ShapesMap::ShapesMap() : bbox(boost::geometry::make_inverse<Box>()) {}
...
void ShapesMap::add_wkt(std::string feature) {
    Polygon poly;
...
auto ShapesMap::bottom_left() const -> Position { return tuplify(bbox.min_corner()); }

auto ShapesMap::top_right() const -> Position { return tuplify(bbox.max_corner()); }
...
auto ShapesMap::min_distance_towards(Position p, double heading, coord_t maximum_range,
                                     coord_t noise) -> std::tuple<coord_t, Position> {
    ...
    for (auto &idx : query_result) {
        ...
        if (fast_intersect(maximum_range, p, end, segm.first, segm.second, tmp, dist)) {
            if (dist < min_dist) {
                min_dist = dist;
                touch_point = tmp;
            }
    ...
    return std::make_tuple(min_dist + noise,
                           Position{std::get<0>(touch_point) + noise * cos(heading),
