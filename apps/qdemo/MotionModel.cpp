...
double get_x(const Pose &p) { return p.position().x(); }

double get_y(const Pose &p) { return p.position().y(); }

double get_heading(const Pose &p) { return p.orientation(); }

MotionSample::MotionSample() : m_startPose(new Pose({5, 5}, 0.0)) {}

MotionSample::MotionSample(QPointF pos, double dir) : m_startPose(new Pose(pos, dir)) {}
...
MotionModel::MotionModel()
    : m_rotationMotion({5, 5}, 0.0), m_straightMotion({5, 5}, 1.57079632679) {}

std::array<double, 4> MotionModel::params() {
    return {m_a0, m_a1, m_a2, m_a3};
}
