...
template <> double get_velocity(const VelocityControl &p) { return p.v(); }

template <> double get_rotation(const VelocityControl &p) { return p.w(); }

template <> double get_time(const VelocityControl &p) { return p.t(); }
}
...
Simulation::Simulation() : m_currentPose(new Pose()) {}
