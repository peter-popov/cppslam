...
public:
	...
	void setMaxRange(double v) { if (m_maxRange == v) return; m_maxRange = v; recalculate();}
	void setLambda(double v) { if (m_lambda == v) return; m_lambda = v; recalculate();}
	void setSigma(double v) { if (m_sigma == v) return; m_sigma = v; recalculate();}
