...
class MclSettings : public QObject {
  ...
  public:
    enum ResamplingType { Multimodal, Stratified, Systematic };
...
template <typename T> static T *list_at(QQmlListProperty<T> *property, int index) {
    auto lst = (QList<std::shared_ptr<T> > *)property->data;
    return lst->at(index).get();
}

template <typename T> static int list_count(QQmlListProperty<T> *property) {
...
class Simulation : public QObject {
    ...
    Q_PROPERTY(SensorSettingsModel *sensorModel READ sensorModel WRITE setSensorModel NOTIFY
                   sensorModelChanged)
    Q_PROPERTY(MotionModel *motionModel READ motionModel WRITE setMotionModel NOTIFY
                   motionModelChanged)
  ...
  public slots:
    ...
    void setMcl(MclSettings *v) {
        m_mcl = v;
        emit mclChanged(v);
    }
    void setSensorModel(SensorSettingsModel *v) {
        m_sensorModel = v;
        emit sensorModelChanged(v);
    }
    void setMotionModel(MotionModel *v) {
        m_motionModel = v;
        emit motionModelChanged(v);
    }
